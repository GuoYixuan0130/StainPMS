...
def do_pool(x: torch.Tensor, pool: nn.Module, norm: nn.Module = None) -> torch.Tensor:
    ...
    return x
from sam2_train.modeling.backbones.MFRB import MFRB
...
class Hiera(nn.Module):
    ...
    def __init__(
        ...
        self.input_type = 'fft' 
        self.freq_nums = 0.25 
        self.handcrafted_tune = True  # tine tune the handcrafted prompt
        self.embedding_tune = True # tune the embedding prompt
        self.adaptor = 'fully_unshared' 
        self.prompt_generator = PromptGenerator(self.scale_factor, self.prompt_type, self.embed_dim,
                                                ...
                                                img_size, num_heads, self.use_fusion, self.window_spec, self.window_pos_embed_bkg_spatial_size)
    ...
    def _get_pos_embed(self, pos_embed, window_embed, hw: Tuple[int, int]) -> torch.Tensor:
        h, w = hw
    ...
    def forward(self, x: torch.Tensor,point_feature: List[torch.Tensor]) -> List[torch.Tensor]:
        inp = x
        ...
        a,b,c,d = self.stage_ends[0], self.stage_ends[1], self.stage_ends[2], self.stage_ends[3]
        for i, blk in enumerate(self.blocks): # stages: (2, 3, 16, 3)
            if i < a+1:
                self.block1.append(blk)  # 第一个块包含前2个元素
            elif a < i < b+1:
                self.block2.append(blk)  # 第二个块包含接下来的3个元素
            elif b < i < c+1:
                self.block3.append(blk)  # 第三个块包含接下来的16个元素
            elif c < i:
                self.block4.append(blk)  # 其余元素组成第四个块

        # Add pos embed
        x = x + self._get_pos_embed(self.pos_embed,self.pos_embed_window,x.shape[1:3])
        if '1' in self.tuning_stage:
            prompt1 = self.prompt_generator.init_prompt(x, handcrafted1, 1)
        for i, blk in enumerate(self.block1):
            if self.use_fusion and '1' in self.tuning_stage:
                x,point_feat = self.prompt_generator.get_prompt(x, point_feature[0], prompt1, 1, i)
            x = blk(x)
        # x = self.norm1(x)
            if i == self.depth[0]-1:
        ...
        for i, blk in enumerate(self.block2):
            if self.use_fusion and '2' in self.tuning_stage:
                if i == 0:
                    prompt2 = self.prompt_generator.init_prompt(x, handcrafted1, 1)
                    x,point_feat = self.prompt_generator.get_prompt(x, point_feature[0], prompt2, 2, i)
                elif i == 1:
                    prompt2 = self.prompt_generator.init_prompt(x, handcrafted2, 2)
                    x,point_feat = self.prompt_generator.get_prompt(x, point_feature[1], prompt2, 2, i)
            x = blk(x)
        # x = self.norm2(x)
            if i == self.depth[1]-1:
                feat = x.permute(0, 3, 1, 2)
        ...
        for i, blk in enumerate(self.block3):
            if self.use_fusion and '3' in self.tuning_stage:
                if i == 0:
                    prompt3 = self.prompt_generator.init_prompt(x, handcrafted2, 2)
                    x,point_feat = self.prompt_generator.get_prompt(x, point_feature[1], prompt3, 3, i)
                elif i == 1:
                    prompt3 = self.prompt_generator.init_prompt(x, handcrafted3, 3)
                    x,point_feat = self.prompt_generator.get_prompt(x, point_feature[2], prompt3, 3, i)
        ...
        for i, blk in enumerate(self.block4):
            if self.use_fusion and '4' in self.tuning_stage:
                if i == 0:
                    prompt4 = self.prompt_generator.init_prompt(x, handcrafted3, 3)
                    x,point_feat = self.prompt_generator.get_prompt(x, point_feature[2], prompt4, 4, i)
                elif i == 1:
                    prompt4 = self.prompt_generator.init_prompt(x, handcrafted4, 4)
                    x,point_feat = self.prompt_generator.get_prompt(x, point_feature[3],prompt4, 4, i)
...
class PromptGenerator(nn.Module):
    def __init__(self, scale_factor, prompt_type, embed_dims, tuning_stage, depths, input_type,
                 freq_nums, handcrafted_tune, embedding_tune, adaptor, img_size, num_heads, use_fion,window_spec,window_pos_embed_bkg_spatial_size):
        """
        Args:
        """
        ...
        self.num_heads = num_heads
        self.k = 2
        self.use_fion = use_fion
        self.window_spec = window_spec
        self.window_pos_embed_bkg_spatial_size = window_pos_embed_bkg_spatial_size

        if self.use_fion:
            self.pos_embed1 = nn.Parameter(
                torch.zeros(1, embed_dims[0], *self.window_pos_embed_bkg_spatial_size)
            )
            self.pos_embed2 = nn.Parameter(
                torch.zeros(1, embed_dims[1], *self.window_pos_embed_bkg_spatial_size)
            )
            self.pos_embed3 = nn.Parameter(
                torch.zeros(1, embed_dims[2], *self.window_pos_embed_bkg_spatial_size)
            )
            self.pos_embed4 = nn.Parameter(
                torch.zeros(1, embed_dims[3], *self.window_pos_embed_bkg_spatial_size)
            )
            self.pos_embed_window1 = nn.Parameter(
                torch.zeros(1, embed_dims[0], self.window_spec[0], self.window_spec[0])
            )
            self.pos_embed_window2 = nn.Parameter(
                torch.zeros(1, embed_dims[1], self.window_spec[1], self.window_spec[1])
            )
            self.pos_embed_window3 = nn.Parameter(
                torch.zeros(1, embed_dims[2], self.window_spec[2], self.window_spec[2])
            )
            self.pos_embed_window4 = nn.Parameter(
                torch.zeros(1, embed_dims[3], self.window_spec[3], self.window_spec[3])
            )
            ...
            for i in range(self.depths[0]):
                ...
                if self.use_fion:
                    ...
                    for j in range(self.k):
                        layers.append(
                            MFRB(
                                embedding_dim=embed_dims[0],
                                num_heads=num_heads,
                            )
                        )
                    setattr(self, 'layers1_{}'.format(str(i)), layers)

                    conv1x1 = nn.Conv2d(in_channels=256, out_channels=embed_dims[0], kernel_size=1)
                    setattr(self, 'conv1x1_1_{}'.format(str(i)), conv1x1)
            for i in range(self.depths[1]):
                if(i == 0):
                    ...
                    if self.use_fion:
                        ...
                        setattr(self, 'layers2_{}'.format(str(i)), layers)

                        conv1x1 = nn.Conv2d(in_channels=256, out_channels=embed_dims[0], kernel_size=1)
                        setattr(self, 'conv1x1_2_{}'.format(str(i)), conv1x1)
                else:
                    fully_unshared_mlp1 = nn.Sequential(
                        nn.Linear(self.embed_dims[1] // self.scale_factor, self.embed_dims[1] // self.scale_factor),
                        nn.GELU(),
                    ...
                    if self.use_fion:
                        ...
                        for j in range(self.k):
                            layers.append(
                                MFRB(
                                    embedding_dim=embed_dims[1],
                                    num_heads=num_heads,
                                )
                            )
                        setattr(self, 'layers2_{}'.format(str(i)), layers)

                        conv1x1 = nn.Conv2d(in_channels=256, out_channels=embed_dims[1], kernel_size=1)
                        setattr(self, 'conv1x1_2_{}'.format(str(i)), conv1x1)
            ...
            for i in range(self.depths[2]):
                if(i == 0):
                    ...
                    if self.use_fion:
                        ...
                        setattr(self, 'layers3_{}'.format(str(i)), layers)

                        conv1x1 = nn.Conv2d(in_channels=256, out_channels=embed_dims[1], kernel_size=1)
                        setattr(self, 'conv1x1_3_{}'.format(str(i)), conv1x1)
                else:
                    fully_unshared_mlp1 = nn.Sequential(
                        nn.Linear(self.embed_dims[2] // self.scale_factor, self.embed_dims[2] // self.scale_factor),
                        nn.GELU(),
                    ...
                    if self.use_fion:
                        ...
                        for j in range(self.k):
                            layers.append(
                                MFRB(
                                    embedding_dim=embed_dims[2],
                                    num_heads=num_heads,
                                )
                            )
                        setattr(self, 'layers3_{}'.format(str(i)), layers)

                        conv1x1 = nn.Conv2d(in_channels=256, out_channels=embed_dims[2], kernel_size=1)
                        setattr(self, 'conv1x1_3_{}'.format(str(i)), conv1x1)
            ...
            for i in range(self.depths[3]):
                if(i == 0):
                    ...
                    if self.use_fion:
                        ...
                        setattr(self, 'layers4_{}'.format(str(i)), layers)

                        conv1x1 = nn.Conv2d(in_channels=256, out_channels=embed_dims[2], kernel_size=1)
                        setattr(self, 'conv1x1_4_{}'.format(str(i)), conv1x1)
                else:
                    fully_unshared_mlp1 = nn.Sequential(
                        nn.Linear(self.embed_dims[3] // self.scale_factor, self.embed_dims[3] // self.scale_factor),
                        nn.GELU(),
                    ...
                    if self.use_fion:
                        ...
                        for j in range(self.k):
                            layers.append(
                                MFRB(
                                    embedding_dim=embed_dims[3],
                                    num_heads=num_heads,
                                )
                            )
                        setattr(self, 'layers4_{}'.format(str(i)), layers)
                        conv1x1 = nn.Conv2d(in_channels=256, out_channels=embed_dims[3], kernel_size=1)
                        setattr(self, 'conv1x1_4_{}'.format(str(i)), conv1x1)
    ...
    def get_prompt(self, x, point_feat, prompt, block_num, depth_num):
        point_feature = 0
        feat = 0
        ...
        if self.use_fion:
            if block_num == 1:
                ...
                pos_embed_window = self.pos_embed_window1
            elif block_num == 2:
                if depth_num == 0:
                    pos_embed = self.pos_embed1
                    pos_embed_window = self.pos_embed_window1
                    ...
                    pos_embed_window = self.pos_embed_window2
            elif block_num == 3:
                if depth_num == 0:
                    pos_embed = self.pos_embed2
                    pos_embed_window = self.pos_embed_window2
                    ...
                    pos_embed_window = self.pos_embed_window3
            elif block_num == 4:
                if depth_num == 0:
                    pos_embed = self.pos_embed3
                    pos_embed_window = self.pos_embed_window3
                else:
                    pos_embed = self.pos_embed4
                    pos_embed_window = self.pos_embed_window4

            conv1x1 = getattr(self, 'conv1x1_' + str(block_num) + '_' + str(depth_num))
            layers = getattr(self, 'layers' + str(block_num) + '_' + str(depth_num))

            feat = x.permute(0, 3, 1, 2)
            b, c, h, w = feat.shape
            queries = conv1x1(point_feat).flatten(2).permute(0, 2, 1)
            keys = feat.flatten(2).permute(0, 2, 1)
            keys_pe = self._get_pos_embed(pos_embed,pos_embed_window, feat.shape[2:4]).permute(0, 3, 1, 2).flatten(2).permute(0, 2, 1)
            for layer in layers:
                queries, keys = layer(
                    point=queries,
                    point_pe = None,
            ...
            x = keys.transpose(1, 2).view(b, c, h, w).permute(0, 2, 3, 1)
            point_feature = queries.transpose(1, 2).view(b, c, h, w)
        return x, point_feature
